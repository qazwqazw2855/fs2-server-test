...
Set-Location $repo
$runDir='C:\Users\SeiHo\Desktop\Simao\God2\God2 Classic Server\Artifacts\ClientInstrumentation\OfficialEvidenceLauncher\prepared-20260817-172610'
...
$clientExe='C:\Users\SeiHo\Desktop\XJZ2\God2_opt.exe'
$clientRoot='C:\Users\SeiHo\Desktop\XJZ2'
...
$result=[ordered]@{ exitCode=$exit; output=@($output); runDir=$runDir; pid=if($pidLine){[int]($pidLine -replace '^PID=','')}else{$null}; module=if($moduleLine){($moduleLine -replace '^MODULE=','')}else{$null}; generalLog=(Join-Path $runDir 'general.log'); metadata=(Join-Path $runDir 'metadata.jsonl') }
