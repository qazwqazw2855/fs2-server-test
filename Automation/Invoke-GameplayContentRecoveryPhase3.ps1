...
try {
    ...
    $toolArguments = @("--repository-root", $repoRoot, "--client-root", $ClientRoot, "--phase", "phase3")
    if ($ScanOnly) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Gameplay content recovery Phase 3 exited with code $LASTEXITCODE."
    }

    if (-not $ScanOnly -and -not $SkipExternalGates) {
        & (Join-Path $repoRoot "Automation\Finalize-GameplayContentRecoveryPhase3Gates.ps1") -RepositoryRoot $repoRoot
    }
}
finally {
