...
$artifactRoot = Join-Path $repoRoot "Artifacts\GameplayContentRecoveryPhase3"
$summaryPath = Join-Path $artifactRoot "final-summary.json"
$testResultsPath = Join-Path $artifactRoot "test-results.json"
$reportPath = Join-Path $repoRoot "Reports\GameplayContentRecoveryPhase3.Final.md"
if (-not (Test-Path -LiteralPath $summaryPath) -or -not (Test-Path -LiteralPath $testResultsPath)) {
    throw "Phase 3 content-recovery artifacts are missing; external gates were not run."
}
...
if ([string]::IsNullOrWhiteSpace($recoveryRunId)) {
    throw "Phase 3 final summary does not contain a recovery run id."
}

$gateStartedAtUtc = [DateTimeOffset]::UtcNow
Push-Location $repoRoot
$temporaryResults = Join-Path ([IO.Path]::GetTempPath()) ("god2-phase3-gates-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    & dotnet test $solution -c Release --no-build --nologo --results-directory $temporaryResults --logger "trx;LogFilePrefix=phase3-gate"
    ...
    if ($networkEmitApiMatches -ne 0) {
        throw "Content-recovery network-emission gate found $networkEmitApiMatches API matches."
    }
