...
$scratchRoot = [IO.Path]::GetFullPath(
    (Join-Path $repoRoot "Artifacts\AnalysisScratch\BattleActorSnapshots"))
...
if (-not $target.StartsWith($scratchPrefix, [StringComparison]::OrdinalIgnoreCase)) {
    throw "Snapshot scratch purge boundary failed."
}

$manifestPath = Join-Path $target "manifest.json"
if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
    throw "Readable snapshot manifest is missing."
}
$manifest = Get-Content -LiteralPath $manifestPath -Raw -Encoding UTF8 |
    ConvertFrom-Json
if ([string]$manifest.sessionId -cne $SessionId -or
    [bool]$manifest.rawPacketDataIncluded) {
    throw "Snapshot manifest does not authorize scratch deletion."
}
