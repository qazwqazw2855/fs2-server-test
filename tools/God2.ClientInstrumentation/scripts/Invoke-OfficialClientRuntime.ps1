...
try {
    ...
    $battleUiPlanSource = Join-Path $repoRoot 'Automation\BattleUiVitalProbePlan.json'
    $battleUiPlanCompiler = Join-Path $repoRoot 'Automation\Invoke-GenericProbePlan.ps1'
    if (-not (Test-Path -LiteralPath $battleUiPlanSource -PathType Leaf) -or
        -not (Test-Path -LiteralPath $battleUiPlanCompiler -PathType Leaf)) {
        throw 'Official Battle UI probe plan or compiler is missing.'
    }
    $startupBattleUiPlan = Join-Path $sensitiveDir `
        'battle-ui-vital-probe-plan.gpp1.bin'
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
        $battleUiPlanCompiler -Action Compile -TargetProcessId $game.Id `
        -PlanPath $battleUiPlanSource -BinaryOut $startupBattleUiPlan | Out-Null
    if ($LASTEXITCODE -ne 0 -or
        -not (Test-Path -LiteralPath $startupBattleUiPlan -PathType Leaf) -or
        (Get-Item -LiteralPath $startupBattleUiPlan).Length -ne 336) {
        throw 'Official Battle UI startup probe plan compilation failed.'
    }
    $arguments = @(
                ...
                if (Test-Path -LiteralPath $ringGeneralLog -PathType Leaf) {
                    try {
                        ...
                        $strictCapabilityReady = $capabilityText -match
                            'evidence capabilities status=STRICT_READY encryptedTransport=0 postDecrypt=1 preEncrypt=1 handlerDecoded=1 battleActorMemory=1 battleStateMemory=1 networkOnly=0 sharedRingRequired=1' -and
                            $capabilityText -match
                            'startup-generic-probe-plan status=Applied generation=20260814 targets=3 active=1 commandStatus=1'
