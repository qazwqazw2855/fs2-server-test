...
The 2026-09-25 read-only activation-candidate audit found that all 167 enabled Formal quests have a nonempty legacy `StepsJson` candidate, while **zero** have a `StartNpcClientId` or `EndNpcClientId` candidate; zero have all three inputs. These step payloads are unverified candidates, and the NPC binding gap prevents a runnable quest start/turn-in flow. This audit did not promote objectives or enable runtime behavior.
