...
CREATE TABLE IF NOT EXISTS `god2_research`.`pet_content_profile_candidate_rows` (
    ...
    KEY `ix_pet_content_profile_candidate_rows_client_pet` (`client_pet_id`),
    KEY `ix_pet_content_profile_candidate_rows_item` (`item_id`),
    KEY `ix_pet_content_profile_candidate_rows_formal_template` (`formal_pet_template_id`),
    KEY `ix_pet_content_profile_candidate_rows_status` (`sync_status_zh_tw`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='寵物 profile 每列可讀候選摘要；用流水號取代 hash，完整保留來源列數但不暴露空 JSON payload';

INSERT INTO `god2_research`.`pet_content_profile_candidate_rows`
...
GRANT SELECT, INSERT, UPDATE, DELETE ON `god2_research`.`pet_content_profile_candidate_rows` TO 'god2_server'@'localhost';
GRANT SELECT, INSERT, UPDATE, DELETE ON `god2_research`.`pet_content_profile_candidate_rows` TO 'god2_server'@'127.0.0.1';
GRANT SELECT, INSERT, UPDATE, DELETE ON `god2_research`.`pet_content_profile_candidate_rows` TO 'god2_catalog_builder'@'localhost';
GRANT SELECT, INSERT, UPDATE, DELETE ON `god2_research`.`pet_content_profile_candidate_rows` TO 'god2_catalog_builder'@'127.0.0.1';
