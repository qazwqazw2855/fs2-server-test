...
SET `direct_runtime_reference` = 1,
    ...
    `runtime_usage_policy_zh_tw` = '正式 runtime catalog 啟動盤點會讀取並指紋化神仙與寵物成長類型表；自動成長算法接入前先保證資料進 release manifest。',
    `cleanup_decision_zh_tw` = '保留，正式 runtime catalog 已對應。'
WHERE `schema_name` = 'god2_game'
  AND `table_name` = 'pet_growth_archetypes';
...
SELECT
    ...
    CASE
        ...
        ELSE '有待接項目'
    END,
    '未直接刪除有正式語意的資料表；神仙與寵物成長類型已納入正式 runtime catalog 盤點與 release manifest。'
