...
SET `direct_runtime_reference` = 1,
    ...
    `runtime_usage_policy_zh_tw` = '正式 runtime catalog 啟動盤點會讀取並指紋化神仙品階表；升階與神仙成長玩法接入前先保證資料進 release manifest。',
    `cleanup_decision_zh_tw` = '保留，正式 runtime catalog 已對應。'
WHERE `schema_name` = 'god2_game'
  AND `table_name` = 'immortal_ranks';
...
SELECT
    ...
    CASE
        ...
        ELSE '有待接項目'
    END,
    '未直接刪除有正式語意的資料表；神仙品階已納入正式 runtime catalog 盤點與 release manifest。'
