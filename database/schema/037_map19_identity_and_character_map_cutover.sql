-- Promote the exact Map 19 resource identity proven by official Client FileIO and the
-- hash-pinned coordinate consumer. The formal database map id is intentionally distinct
-- from the official Client's 10-bit map id.
INSERT INTO `maps` (
...
WHERE evidence.`Domain` = 'Map'
  ...
  AND JSON_UNQUOTE(JSON_EXTRACT(evidence.`NormalizedData`, '$.resourceName')) = 'indoor/groceryl.hmd'
  AND (
      SELECT COUNT(DISTINCT CONCAT(candidate.`SourceHash`, ':', candidate.`NormalizedHash`))
      FROM `content_validated_records` candidate
      WHERE candidate.`Domain` = 'Map'
        AND candidate.`AuthorityKey` = evidence.`AuthorityKey`
  ) = 1
...
ORDER BY evidence.`ValidatedAtUtc` DESC, evidence.`RunId` DESC
LIMIT 1;
...
CREATE TABLE IF NOT EXISTS `character_map_identity_migrations` (
    `CharacterId` bigint NOT NULL,
    `ClientBuildId` varchar(64) NOT NULL,
    `LegacyClientMapId` int NOT NULL,
    `DatabaseMapId` int NOT NULL,
    `PositionX` int NOT NULL,
    `PositionY` int NOT NULL,
    `EvidenceReference` varchar(512) NOT NULL,
    `MigratedAtUtc` datetime(6) NOT NULL,
    PRIMARY KEY (`CharacterId`, `ClientBuildId`),
    KEY `IX_CharacterMapIdentityMigrations_DatabaseMap` (`DatabaseMapId`),
    CONSTRAINT `FK_CharacterMapIdentityMigrations_Character`
        FOREIGN KEY (`CharacterId`) REFERENCES `characters` (`Id`),
    CONSTRAINT `FK_CharacterMapIdentityMigrations_Map`
        FOREIGN KEY (`DatabaseMapId`) REFERENCES `maps` (`Id`)
